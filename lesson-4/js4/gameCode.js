...
function startGame() {
    ...
    console.log(answers);
    if (answers.length > 0) {
        steps = 'Было записано ' + answers.length + ' шаг(а) квеста <br>' +
                    '<label>Выберите какой шаг вы хотите посмотерть введя от 1 до ' + answers.length + ':</label>' +
                    '<input type="number" class="step" min="1" max="' + answers.length + '" step="1">' +
...
function checkStep() {
    ...
    steps += '<hr><p>Вопрос на шаге ' + step + ': ' + answers[step-1].question + '</p>' +
             '<p>Ответ на шаге ' + step + ': ' + answers[step-1].answer + '</p>';
    document.getElementsByClassName("steps")[0].innerHTML = steps;
    if (step > 0) {
